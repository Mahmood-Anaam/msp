...
if TYPE_CHECKING:
    ...
    from transformers.tokenization_python import PreTrainedTokenizerBase
...
class MSPProcessorWithLM(ProcessorMixin):
    def __init__(
        self,
        ...
        super().__init__(feature_extractor, video_processor, tokenizer)
        ...
        if feature_extractor.__class__.__name__ not in ["MSPAudioFeatureExtractor"]:
            raise ValueError(
                f"`feature_extractor` has to be of type `MSPAudioFeatureExtractor`, but is {type(feature_extractor)}"
            )
        if video_processor.__class__.__name__ not in ["MSPVisualVideoProcessor"]:
            raise ValueError(
                f"`video_processor` has to be of type `MSPVisualVideoProcessor`, but is {type(video_processor)}"
            )
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, **kwargs):
        ...
        feature_extractor, video_processor, tokenizer = (
            super()._get_arguments_from_pretrained(
    ...
    def __call__(self, audio=None, videos=None, text=None, **kwargs):
